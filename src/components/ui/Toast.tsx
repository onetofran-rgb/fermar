  ...
  return (
    <div className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 w-80">
      {toasts.map(t => {
        ...
        return (
          <div key={t.id} className={clsx('flex items-start gap-3 rounded-xl border p-4 shadow-lg', cls)}>
            <Icon className="h-5 w-5 mt-0.5 flex-shrink-0" />
            <p className="flex-1 text-sm font-medium">{t.mensaje}</p>
            <button onClick={() => cerrar(t.id)} className="flex-shrink-0 opacity-70 hover:opacity-100">
              <X className="h-4 w-4" />
            </button>
          </div>
        );
